...
class ResNetPolicy(nn.Module):
    ...
    def forward(self, states, image, image_tilted, depth_predicted):
        """Forward pass through the model."""    
        rgbd_feature = self.rgbd_encoder(torch.cat((image, image_tilted, depth_predicted), dim=1)).squeeze(dim=-1).squeeze(dim=-1)
        ...
        return student_path_local, collison_prediction, altitude_prediction
